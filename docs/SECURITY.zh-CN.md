...
| 远端仓库的**写**权限 | **什么都拿不到** —— 见 §5。被推进 `recipients.json` 的公钥只会出现在目录里，永远不会被加密到。 | —— *（修复前：全部，且追溯既往）* |
...
**14 条可行动的发现已全部修复**（多数带走回归测试），另外 2 条是已记录的取舍，不是缺陷。唯一需要设计改动而非打补丁的那条——远端写权限换取明文——见 §5。
...
| ID | 严重度 | 发现 | 状态 |
|---|---|---|---|
| **A1** | **严重** | 远端写权限 → 收件人注入 → 追溯既往的全量明文 | **已修 + 测试** —— 见 §5 |
...
| A14 | 信息 | 死面：`Config.reveal_allowed` 有文档却从不被读；`--debug` 与 `doctor --agent` 无任何效果；赋值解析拒绝设计文档写的 `field[[type]]` 形式 | 已修 |
...
## 5. A1 —— 远端写权限，以及它是怎么被堵上的

`recipients.json` 由远端分发、被**无条件采纳**。任何能写它的人加一个公钥，下一次合法写入就会把**整个金库（含历史）**重新加密给那把钥匙。这击穿了设计的核心承诺：远端在**读取**上被视为不可信，却在**写入**上被默认信任。

**修法：本地信任集合。**

`recipients.json` 现在只回答「谁存在」。第二份集合放在 `~/.config/akey/config.toml` 里、**永不同步**，回答「谁被允许解密」。加密用二者的交集。

- `Store::save_with` 照旧写全量目录（设备之间要能看见彼此），但**只加密给本机批准过的公钥**。
- 出现在仓库里却未获批准的公钥，会被 `akey sync` 与 `akey doctor` 报为 **pending**，并且什么也拿不到。
- `akey devices trust <名字|公钥>` 批准一个，并**立刻**把当前金库重新加密给它，不必等下一次写入；`akey devices untrust` 反向撤回。
- `akey init` 信任本机；`akey init --from` 信任**引导那一刻已经在库里**的收件人——它们正是能打开运营者刚刚交出恢复密码的那个金库的一组人，所以交出密码这个动作本身就是信任锚。引导**之后**才出现的公钥永不自动信任。
- `akey devices rm` 同时把它从目录与信任集合里拿掉。

**代价**是每加一台设备，要在**其他每台设备**上多跑一条命令：

```bash
akey devices trust laptop
```

**回归测试**：`tests/e2e_sync.rs::an_injected_recipient_never_receives_ciphertext` 完整重放这次攻击——克隆远端、加公钥、推送——并断言注入的公钥被报为 pending，且在受害者再次写入之后仍然够不着（退出码 4）。

**残余（R8）**：在注入**之后**才引导的设备，会通过「信任当时已有的那些」这条规则继承被注入的公钥。要堵死它，需要加入方证明自己确实解开了金库——而在没有签名原语的情况下（age 的 X25519 不做签名），这等于要求每台既有设备各自去验证恢复密码。未实现，如实记录。
...
| R8 | 在注入之后才引导的设备会继承被注入的公钥 | 见 §5。要彻底关闭需要签名原语 |
